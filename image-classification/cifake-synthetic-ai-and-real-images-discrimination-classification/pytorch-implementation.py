...
# data loading
RAW_DATA = ImageFolder(root="/kaggle/input/cifake-real-and-ai-generated-synthetic-images/train", transform=TRANSFORMS)
  ...
  # code for early stopping, criterion = no accuracy increase for 5 epochs
    if epoch == 0 or (acc > highestAccuracy and acc != float(1)):
        ...
        if highestAccuracy != float(1):
            torch.save(MODEL, "cifake.pt")  # model saved everytime a higher accuracy is attained, however float(1) is discarded, as the model is probably overfitted at that accuracy
